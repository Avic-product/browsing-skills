...
- **With skill:** loads the site action index plus exactly one action reference, then runs the maintained action code.
...
1. Identify the website, action, target URL/query, browser layer, expected output fields, and success criteria.
2. Read only the matching site `SKILL.md` and action reference from `skills/<domain>/references/<action>.md`.
