# Walmart.com — Add To Cart Reference

## Requirements

**Browser:** Required. Use only after the user explicitly chooses an item and quantity.

**Safety:** Write action. It schedules the Add to cart click and stops. It never proceeds to checkout, pickup reservation, payment, or order placement. Run `cart-data` after navigation to verify.

## Action: add-to-cart

**Navigate to:** a Walmart product page for the selected item.
...
({
  ...
  description: "Add an explicitly requested Walmart item to cart without checking out",
  inputSchema: {
    type: "object",
    properties: {
      itemId: { type: "string" },
  ...
  execute: function(params) {
    ...
    function pageItemId() { return (location.href.match(/\/ip\/[^/?#]+\/([0-9]+)/) || [])[1] || ""; }
    ...
    var data = { url: location.href, itemId: itemId || pageItemId(), quantity: quantity, attempted: false, clickScheduled: false, message: "", warnings: [] };
    if (itemId && pageItemId() && itemId !== pageItemId()) {
      data.message = "Current Walmart product page itemId does not match requested itemId.";
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (/add to cart|add item|add/i.test(label) && visible(buttons[i]) && !buttons[i].disabled) { button = buttons[i]; break; }
    }
    if (!button) {
      data.message = "No enabled Add to cart control was visible.";
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    data.attempted = true;
...
**Returns:** `{ url, itemId, quantity, attempted, clickScheduled, message, warnings }`
